-- ============================================================
-- MIGRATION: Add foodpanda_price & grab_price to inventory_items and menu_items
-- ============================================================

-- 1. Add platform price columns to inventory_items and menu_items
ALTER TABLE public.inventory_items
    ADD COLUMN IF NOT EXISTS foodpanda_price NUMERIC CHECK (foodpanda_price >= 0),
    ADD COLUMN IF NOT EXISTS grab_price NUMERIC CHECK (grab_price >= 0);

ALTER TABLE public.menu_items
    ADD COLUMN IF NOT EXISTS foodpanda_price NUMERIC CHECK (foodpanda_price >= 0),
    ADD COLUMN IF NOT EXISTS grab_price NUMERIC CHECK (grab_price >= 0);

-- 2. Update trigger function fn_sync_inventory_to_menu_items to propagate platform prices during direct POS sync
...
BEGIN
    ...
    IF NEW.selling_price IS NOT NULL AND NEW.selling_price > 0 AND NEW.status = 'active' THEN
        INSERT INTO public.menu_items (
            name, sku, category, price, cost_price, type, inventory_item_id, status, tenant_id, available_branches, foodpanda_price, grab_price
        )
        VALUES (
            NEW.item_name, NEW.sku, NEW.category, NEW.selling_price, NEW.cost_per_base_unit, 'retail', NEW.id, NEW.status, v_tenant_id, NEW.available_branches, NEW.foodpanda_price, NEW.grab_price
        )
        ON CONFLICT (inventory_item_id) DO UPDATE SET
            ...
            foodpanda_price = EXCLUDED.foodpanda_price,
...
-- 3. Redefine fn_create_inventory_item to accept p_foodpanda_price and p_grab_price parameters
...
BEGIN
    ...
    INSERT INTO public.inventory_items (
        ...
        conversion_factor, reorder_level, cost_per_base_unit, status, selling_price, available_branches, foodpanda_price, grab_price
    )
    VALUES (
        p_sku, p_item_name, p_category, p_base_unit, p_purchase_unit,
        p_conversion_factor, p_reorder_level, p_cost_per_base_unit, 'active', p_selling_price, p_available_branches, p_foodpanda_price, p_grab_price
    )
