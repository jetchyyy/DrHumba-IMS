-- ============================================================
-- MIGRATION: Fix POS Discount Total Calculation for Philippine BIR Compliance
-- ============================================================

-- 1. Update public.fn_process_sale to accurately compute net payable total
...
BEGIN
    ...
    -- BIR Compliant Net Amount Due (Payable Total)
    v_net_total := v_sum_vatable + v_sum_vat_amount + v_sum_vat_exempt - v_sum_discount;

    IF p_payment_method = 'cash' AND p_amount_tendered IS NOT NULL THEN
        IF p_amount_tendered < (v_net_total - 0.009) THEN
            RAISE EXCEPTION 'Insufficient tender: total is %, tendered is %', v_net_total, p_amount_tendered;
        END IF;
        v_change := GREATEST(0, p_amount_tendered - v_net_total);
    ELSE
        v_change := 0;
    END IF;
    ...
    SET total_amount    = v_net_total,
        ...
        sc_pwd_discount  = CASE WHEN p_discount_type IN ('sc', 'pwd', 'mixed', 'mov') THEN v_sum_discount ELSE 0 END,
        other_discounts  = CASE WHEN p_discount_type NOT IN ('sc', 'pwd', 'mixed', 'mov') THEN v_sum_discount ELSE 0 END
    WHERE id = v_sale_id;
...
-- 2. Update public.fn_process_offline_sale
    ...
    FOR r_item IN (
        ...
        ELSIF v_inventory_item_id IS NOT NULL THEN
            ...
            INSERT INTO public.inventory_movements
                (branch_id, item_id, quantity, movement_type, reference_id, reference_type, created_by, tenant_id)
            VALUES
                (p_branch_id, v_inventory_item_id, -v_qty_needed, 'sale_deduction', v_sale_id, 'sale', p_cashier_id, v_tenant_id);
