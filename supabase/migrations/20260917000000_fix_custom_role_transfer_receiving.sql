-- Migration: Fix custom role transfer receiving permissions and remove invalid requested_by block
-- Created: 2026-09-17

-- 1. Create get_my_allowed_tabs helper function
CREATE OR REPLACE FUNCTION public.get_my_allowed_tabs()
RETURNS TEXT[] AS $$
    SELECT allowed_tabs FROM public.profiles WHERE id = auth.uid();
$$ LANGUAGE sql SECURITY DEFINER STABLE;

-- 2. Redefine fn_receive_transfer to ensure custom roles and requested_by users can confirm receipt
...
BEGIN
    ...
    -- Strict rule: The person confirming must NOT be the sender/approver who dispatched from source
    ...
    -- Check permissions: receiver must belong to target branch, or be super admin / inventory manager, or have 'transfers' in allowed_tabs
    IF v_my_role IS NULL OR (
        v_my_role != 'super_admin' AND 
        v_my_role != 'inventory_manager' AND 
        v_my_branch != v_target AND
        NOT ('transfers' = ANY(COALESCE(v_my_allowed_tabs, ARRAY[]::TEXT[])))
    ) THEN
    ...
    FOR r_item IN (
        ...
        VALUES (v_target, r_item.item_id, r_item.quantity_base_unit, now(), v_tenant_id)
        ...
        VALUES (v_target, r_item.item_id, r_item.quantity_base_unit, 'transfer_in', p_transfer_id, 'transfer', auth.uid(), v_tenant_id);
    END LOOP;
...
-- 3. Redefine fn_submit_transfer_receipt for discrepancy submittals with custom role support
